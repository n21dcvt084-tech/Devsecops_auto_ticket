...
DEFAULT_LIMIT = int(os.getenv("TEST_FINDINGS_LIMIT", "5"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "the normal backend flow, but force SMTP delivery to local Mailpit."
        )
    )
    parser.add_argument(
        "--limit",
        ...
        help="Maximum number of findings to process. Default: TEST_FINDINGS_LIMIT or 5.",
    )
    parser.add_argument(
        "--product-name",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "test, for example: product-a,product-b,product-c."
        ),
    )
    parser.add_argument(
        "--per-product-limit",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional recipient override for Mailpit tests. If omitted, the normal "
            "project mapping or DefectDojo ticket_email is used."
        ),
    )
    parser.add_argument(
        "--clear-db",
    ...
    parser.add_argument(
        "--mailpit-host",
        default=os.getenv("MAILPIT_SMTP_HOST", "mailpit"),
        help="Mailpit SMTP host. Use 'mailpit' in Docker, 'localhost' from local venv.",
    )
    parser.add_argument(
        "--mailpit-port",
        type=int,
        default=int(os.getenv("MAILPIT_SMTP_PORT", "1025")),
...
def apply_mailpit_safety(settings: Settings, host: str, port: int) -> None:
    ...
    for _ in range(max_pages):
        ...
        for raw_finding in raw_findings:
            ...
            if product_name and finding.product != product_name:
                continue
            if recipient:
                finding.ticket_email = recipient
...
def main() -> int:
    ...
    settings = Settings()
    apply_mailpit_safety(settings, args.mailpit_host, args.mailpit_port)
    init_database()
    ...
    print("DONE_PROCESS_ACTIVE_VERIFIED_FINDINGS_TO_MAILPIT")
    print(f"processed_count={len(findings)}")
    print(f"mailpit_url=http://localhost:8025")
    print(f"smtp_target={args.mailpit_host}:{args.mailpit_port}")
    return 0
